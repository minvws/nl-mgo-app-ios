...
public class PdfDrawElementFactory {
	...
	@MainActor public func createPdfHeadingDrawElement(
		...
		let textHeight = text.boundingRect(
			with: CGSize(width: Constants.contentSize.width, height: .greatestFiniteMagnitude),
		...
		return PdfDrawElement(
			...
			rect: CGRect(
				...
				y: currentY,
				width: Constants.contentSize.width,
				height: textHeight
			),
			height: textHeight
		)
	...
	@MainActor public func createPdfSubHeadingDrawElement(
		...
		return PdfDrawElement(
			...
			borderColor: nil,
			rect: CGRect(
				x: Constants.outerMargin + Constants.contentSize.width - textBox.width,
				y: currentY,
				width: textBox.width,
	...
	@MainActor public func createSubTableRowDrawElement(
		...
		let keyHeight = keyText.boundingRect(
			with: CGSize(width: (Constants.contentSize.width / 2) - 12, height: .greatestFiniteMagnitude),
			options: .usesLineFragmentOrigin,
			context: nil
		).height
		...
		return [
			PdfDrawElement(
				...
				rect: CGRect(
					x: Constants.outerMargin,
					y: currentY,
					width: Constants.contentSize.width / 2,
					height: textHeight
				),
				height: textHeight + 11
			),
			PdfDrawElement(
				...
				borderColor: theme.border,
				rect: CGRect(
					x: Constants.outerMargin + (Constants.contentSize.width / 2) - 1,
					y: currentY,
					width: (Constants.contentSize.width / 2) + 1,
					height: textHeight
				),
				height: textHeight + 11
			)
	...
	@MainActor public func createFooterElement(_ pdfData: PdfData) -> PdfDrawElement {
		...
		return PdfDrawElement(
			...
			rect: CGRect(
				x: Constants.outerMargin,
				y: Constants.contentSize.height - textHeight,
				width: Constants.contentSize.width,
				height: textHeight
			),
			height: textHeight
	...
	@MainActor public func createPaginationElement(currentPage: Int, totalPages: Int) -> PdfDrawElement {
		...
		return PdfDrawElement(
			...
			rect: CGRect(
				x: Constants.outerMargin + Constants.contentSize.width - textBox.width,
				y: Constants.contentSize.height - textBox.height,
				width: textBox.width,
				height: textBox.height
			),
			height: 0
