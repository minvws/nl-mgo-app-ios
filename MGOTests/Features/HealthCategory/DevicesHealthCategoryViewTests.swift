...
final class DevicesHealthCategoryViewTests: XCTestCase {
	...
	override func setUp() {
		...
		viewModel = DevicesHealthCategoryViewModel(
	...
	func test_stateEmpty() {
		...
		servicesSpies.dataStoreSpy.stubbedGetCategoryIdResult = .success([
			MgoResourceRecord(categoryId: "\(HealthCategories.Category.devices.rawValue)", organizationId: healthcareOrganization.identifier, resources: [], error: false)]
		)
		let content = NavigationView { sut }
	...
	func test_stateList() throws {
		...
		// Given
		let resource = try getResource("zibMedicalDevice")
		servicesSpies.dataStoreSpy.stubbedGetCategoryIdResult = .success([
			MgoResourceRecord(categoryId: "\(HealthCategories.Category.devices.rawValue)", organizationId: healthcareOrganization.identifier, resources: [resource], error: false)]
		)
	...
	func test_search_itemFound() throws {
		...
		// When
		viewModel.searchText = "Gehoorapparaat"
