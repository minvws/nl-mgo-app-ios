...
final class AlertsHealthCategoryViewTests: XCTestCase {
	...
	func test_stateEmpty() {
		...
		servicesSpies.dataStoreSpy.stubbedGetCategoryIdResult = .success([
			MgoResourceRecord(categoryId: "\(HealthCategories.Category.alerts.rawValue)", organizationId: healthcareOrganization.identifier, resources: [], error: false)]
		)
		let content = NavigationView { sut }
	...
	func test_stateList() throws {
		...
		servicesSpies.dataStoreSpy.stubbedGetCategoryIdResult = .success([
			MgoResourceRecord(categoryId: "\(HealthCategories.Category.alerts.rawValue)", organizationId: healthcareOrganization.identifier, resources: [resource], error: false)]
		)
