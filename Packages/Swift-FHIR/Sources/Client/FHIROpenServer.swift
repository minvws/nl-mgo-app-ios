...
open class FHIROpenServer: FHIRMinimalServer {
	...
	*/
	public final func getCapabilityStatement(_ callback: @escaping (_ error: FHIRError?) -> Void) {
		if nil != capability {
			callback(nil)
			return
		}
	...
	var capabilityOperations: [CapabilityStatementRestOperation]?
	
	/** Find operation with given name. */
	func capabilityOperation(_ name: String) -> CapabilityStatementRestOperation? {
