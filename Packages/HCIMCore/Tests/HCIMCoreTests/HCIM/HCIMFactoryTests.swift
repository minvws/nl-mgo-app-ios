...
@MainActor
struct HCIMFactoryTests {
	
	@Test func invalidData_shouldNotMapOntoZibMedicationUse() throws {
		
		// Given
		let resource = try ResourceLoader().getStringResource("invalid")
	...
	@Test func validData_shouldMapOntoZibMedicationUse() throws {
	...
	private func expectMedicationTreatment(_ object: ZibMedicationUse?) {
		...
		#expect(object?.medicationTreatment?.extensionValueOfMgoIdentifierType == nil)
