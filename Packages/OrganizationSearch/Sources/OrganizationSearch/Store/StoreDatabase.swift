...
enum StoreDatabase {
	...
	static func fetchAll(from dbQueue: DatabaseQueue) throws -> [Organization] {
		try dbQueue.read { db in
			let rows = try Row.fetchAll(db, sql: "SELECT * FROM stored_organization ORDER BY rowid")
	...
	static func replace(with organizations: [Organization], in dbQueue: DatabaseQueue) throws {
		try dbQueue.write { db in
		...
		try db.execute(
			sql: """
				INSERT OR IGNORE INTO stored_organization
					(id, addressLine, careTypeDisplay, city,
					 dataServicesJSON, displayName, geoLat, geoLng,
					 postalCode)
				VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
				""",
			arguments: [
